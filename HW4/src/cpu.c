...
int fetch(){ // pc가 가리키는 inst_mem address에서 inst를 가져옴
    return read_from_cache(pc, true, &stats);
}
...
void execute(Instruction decoded_inst, int* ALU_result, int* mem_index, ControlSignals *control){ // 각 타입별 명령어 실행
    ...
    *ALU_result = ALU(decoded_inst.opcode, input1, input2, decoded_inst.s_imm, decoded_inst.shamt, decoded_inst.zero_ext_imm, control->ALUOp, pc);
    ...
    if (control->MemRead || control->MemWrite) {
        ...
        *mem_index = effective_address / sizeof(int); // 메모리 인덱스 계산
        if (*mem_index < 0 || *mem_index >= MEMORY_SIZE) {
            fprintf(stderr, "[execute mem_index계산] Memory access error: Invalid memory index 0x%x at PC=0x%x\n", *mem_index, pc);
            exit(1);
        }
    }
    update_pc();
...
void access_memory(Instruction decoded_inst, int mem_index, int* mem_value, ControlSignals *control, ExecutionStats *stats) {
    int address = mem_index * sizeof(int);

    if (control->MemRead) { // LW
        *mem_value = read_from_cache(address, false, stats);
    } else if (control->MemWrite) { // SW
        write_to_cache(address, reg[decoded_inst.rt], false, stats);
        if (cache_full()) {
            flush_cache(false);
            printf("Cache is full. Flushing all dirty lines to memory.\n");
        }
    }
}
